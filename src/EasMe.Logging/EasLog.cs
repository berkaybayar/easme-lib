...
public class EasLog : IEasLog
{
    ...
    {
        var loglevel = result.Severity.ToLogLevel();
        if (!loglevel.IsLoggable()) return;
        if (result.Errors.Length > 0)
        {
            WriteLog(_LogSource, loglevel, null, $"ErrorCode:{result.ErrorCode}",
    ...
    public void LogResult<T>(ResultData<T> result)
    {
        ...
        WriteLog(_LogSource, loglevel, null, $"ErrorCode:{result.ErrorCode}", $"Data:{result.Data}");
    }
    ...
    public void LogResult(Result.Result result, object message)
    {
        ...
        if (result.Errors?.Length > 0)
        {
            WriteLog(_LogSource, loglevel, null, $"Message:{message}", $"ErrorCode:{result.ErrorCode}",
                $"Errors:{string.Join(",", result.Errors)}");
            return;
        }
    ...
    private static string GetExactLogPath(LogLevel severity)
    {
        ...
        var path = EasLogFactory.Config.SeparateLogLevelToFolder
            ? Path.Combine(EasLogFactory.Config.LogFolderPath, date,
                severity + "_" + EasLogFactory.Config.LogFileName + date + EasLogFactory.Config.LogFileExtension)
            : Path.Combine(EasLogFactory.Config.LogFolderPath,
